...
const Logger = { warn: (m: string) => console.warn(m) };

type VXYZ = {//type used that's either a value array or an xyz
    x?: number,
    y?: number,
    z?: number,
    value?: number[],
...
export class GeometryConverter {
    ...
    public static convertNodeToGeometryData(node: NodeData): GeometryData | null | undefined {
     ...
     * MESH
     */
    private static MeshToGeometryData(node: NodeData): GeometryData | undefined {
        if (!node.raw) return
    ...
    private static PointToGeometryData(node: NodeData): GeometryData {
        ...
        return {
            attributes: {
                POSITION: this.PointToFloatArray(node.raw as VXYZ)
            },
            bakeTransform: new Matrix4().makeScale(
                conversionFactor,
                conversionFactor,
    ...
    private static PointToVector3(obj: VXYZ, scale = true) {
    ...
    private static PointToFloatArray(obj: VXYZ) {
        if (obj.value) {
    ...
    private static srgbToLinear(x: number) {
        if (x <= 0) return 0
