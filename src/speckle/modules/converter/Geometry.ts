...
import type { SpeckleObject } from './types';
...
export class Geometry {
    ...
    static mergeGeometryData(geometries: GeometryData[]): GeometryData {
        ...
        for (let i = 0; i < geometries.length; i++) {
            let bakeTransform = geometries[i].bakeTransform;
            if (bakeTransform) Geometry.transformGeometryData(geometries[i], bakeTransform)
        }
